...
| 测试 | 实现 | 验收 |
|------|------|------|
| `test_lstm_forecast_requires_ml_extra` | 无 `ml` 依赖时导入提示清晰 | - |
| `test_lstm_forecast_horizon`（`@pytest.mark.ml`） | `LstmForecast` | 可选 CI job |
...
1. **迭代 8（M5+）**：深度学习预测策略（`.[ml]`）
2. 扩展 API：`POST /api/v1/crawl`、`GET /api/v1/quotes/{symbol}`
